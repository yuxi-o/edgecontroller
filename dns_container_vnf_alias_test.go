...
var _ = Describe("Entities: DNSContainerVNFAlias", func() {
	var (
		alias *cce.DNSContainerVNFAlias
	)
	...
	Describe("GetTableName", func() {
		It(`Should return "dns_container_vnf_aliases"`, func() {
			Expect(alias.GetTableName()).To(Equal("dns_container_vnf_aliases"))
		})
	})
	...
	Describe("Validate", func() {
		...
		It("Should return an error if ContainerVNFID is not a UUID", func() {
			alias.ContainerVNFID = "123"
			Expect(alias.Validate()).To(MatchError(
				"container_vnf_id not a valid uuid"))
		})
	})
