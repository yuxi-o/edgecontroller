...
var _ = Describe("/container_apps", func() {
	...
	Describe("DELETE /container_apps/{id}", func() {
		...
		DescribeTable("422 Unprocessable Entity",
			func() {
				postDNSContainerAppAliases(containerAppID)

				By("Sending a DELETE /container_apps/{id} request")
				req, err := http.NewRequest(
					http.MethodDelete,
					fmt.Sprintf(
						"http://127.0.0.1:8080/container_apps/%s",
						containerAppID),
					nil)
				Expect(err).ToNot(HaveOccurred())
			...
			Entry("DELETE /container_apps/{id} with dns_container_app_aliases record"), //nolint:lll
		)
	})
})
