		...
		DescribeTable("422 Unprocessable Entity",
			func() {
				...
				Expect(string(body)).To(Equal(fmt.Sprintf(
					"cannot delete dns_container_app_alias_id %s: record in "+
						"use in dns_configs_dns_container_app_aliases",
					dnsContainerAppAliasID)))
			},
			Entry("DELETE /dns_container_app_aliases/{id} with dns_configs_dns_container_app_aliases record"), //nolint:lll
		)
	})
})
